...
# Effective diffusivity model with per-dopant defect calibration.
#
# Boron: FLOOXS_2026 point defects + v5 {311} clustering + v5 surface recomb
#   - Table A-3 effective D with FLOOXS_2026 ScaleI
#   - No Vac PDE: V pathway uses ScaleVac=1 (FLOOXS_2026 D_V is so fast that
#     Vac PDE self-neutralizes IV recomb via rapid V depletion - confirmed by
#     4-PDE test giving identical Xj to 3-PDE)
#   - alpha_311=0.2 (recalibrated from v5's 0.1 because the eff model's
#     D_table*ScaleI is 18% higher per ScaleI than v5's pair-mediated D_eff)
#   - 3 PDEs: boron, Inter, CIc
#
# P/As: TSUPREM-4 Appendix A parameters
...
switch ${dopant} {
    boron {
        ...
        # Kr Ea=3.6 eV (Eaglesham et al.), alpha=0.2 (recalibrated for eff model)
...
# =============================================================================
# 8. IV BULK RECOMBINATION (Eq 3-286, As only)
# =============================================================================

# kIV = 4*pi*(D_I + D_V)*a0 (B has no Vac PDE, P is I-only)
set kIV "(4.0*3.14159*($DiffI+$DiffV)*$lattice)"

# =============================================================================
# 9. PER-DOPANT EFFECTIVE DIFFUSIVITIES (Table A-3)
# =============================================================================

switch ${dopant} {
    boron {
        # I pathway: neutral (DIX) + positive (DIP * Poni)
        # V pathway: neutral (DVX) + positive (DVP * Poni)
        # All Ea = 3.46 eV
        pdbSetDouble Silicon boron DIX {[Arrhenius 0.03517 3.46]}
        pdbSetDouble Silicon boron DIP {[Arrhenius 0.6835 3.46]}
        pdbSetDouble Silicon boron DVX {[Arrhenius 1.850e-3 3.46]}
        pdbSetDouble Silicon boron DVP {[Arrhenius 0.03601 3.46]}
...
switch ${dopant} {
    boron {
        # 3 PDEs: boron, Inter, CIc (no Vac PDE)
...
# EqVac/ScaleVac only for As (B has no Vac PDE, P is I-only)
switch ${dopant} {
    arsenic {
...
# where D_I_dop = sum of charge-state components from Table A-3
...
switch ${dopant} {
    boron {
        ...
        # DiffDopV = (DVX + DVP * Poni) * 1.0 / Poni  (ScaleVac=1, no Vac PDE)
...
# =============================================================================
# 21. INTER EQUATION (Eq 3-280 with pair coupling)
# =============================================================================
...
switch ${dopant} {
    boron {
        ...
        pdbSetString Silicon Inter Equation \
            "ddt(Inter) - $DiffI*EqInter*grad(ScaleInter) - DiffDopI*grad(boron*Poni) + R_clust"
    }
    phosphorus {
        # No IV recombination (no Vac PDE)
        pdbSetString Silicon Inter Equation \
