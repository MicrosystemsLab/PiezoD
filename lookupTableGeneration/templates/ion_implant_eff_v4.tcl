...
# Effective diffusivity model v4: v2 + ScaleInter cap for boron stability.
#
...
#   - Soft ScaleInter cap (SmaxDop=200) in DiffDopI only: prevents flux
#     divergence at extreme supersaturation (2e16 dose + frozen {311} at 900C)
#     I self-diffusion uses real ScaleInter (capping it breaks ramp down)
#   - 10um mesh (captures Xj > 5um), no ramp down (diverges on deep mesh)
...
# Soft ScaleInter cap for boron DiffDopI stability (smooth, differentiable)
# Prevents DiffDopI divergence at extreme supersaturation (2e16 + frozen {311})
# I self-diffusion uses real ScaleInter (capping it suppresses I removal during ramp down)
set SmaxDop 200
term name = ScaleCapDop add eqn = "ScaleInter * $SmaxDop / (ScaleInter + $SmaxDop)" Silicon
...
switch ${dopant} {
    boron {
        # DiffDopI = (DIX + DIP * Poni) * ScaleCapDop / Poni  (tight cap for stability)
        # DiffDopV = (DVX + DVP * Poni) * 1.0 / Poni  (V at equilibrium, ScaleVac=1)
        term name = DiffDopI add eqn = "($DIX_B + $DIP_B * Poni) * ScaleCapDop / Poni" Silicon
...
switch ${dopant} {
    boron {
        ...
        # Real ScaleInter in self-diffusion: physical I transport, uncapped
        # (capping I self-diff suppresses I removal during ramp down at low T)
...
# 26. ANNEAL SEQUENCE
# =============================================================================
# Ramp up included for thermal budget. Ramp down omitted: causes solver
# divergence on 10um mesh (EqInter drops 5700x during 1100C->800C cooldown).
