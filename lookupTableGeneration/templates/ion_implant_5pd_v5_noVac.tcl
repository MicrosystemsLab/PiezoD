...
#   - I-V bulk recombination (P, As only)
...
#       Boron:      B-I pairs only (V pathway ~5%, dropped for stability)
...
#
# v5-noVac: Based on v5. Boron drops Vac PDE, BV pairs, and IV recombination.
# The V pathway is only ~5% of B diffusion, and FLOOXS_2026 D_V (235,000x
# larger than Table A-17) causes V to equilibrate in microseconds, making the
# Vac PDE a no-op anyway. Removing it eliminates the stiffness source that
# crashes v5 at B 1100C 2e16.
# P/As: completely unchanged from v5.
...
# Used by P and As (not boron in noVac variant)
...
switch ${dopant} {
    boron {
        ...
        # No BV pair (V pathway ~5% of B diffusion, dropped for stability)
...
switch ${dopant} {
    boron {
        # 4 vars: boron, boronInt, Inter, CIc (no Vac/BV: dropped for stability)
...
switch ${dopant} {
    boron {
        # No Vac variable for B (V pathway ~5%, dropped for stability).
...
# EqVac and ScaleVac only for As (B dropped in noVac, P is I-only)
switch ${dopant} {
    arsenic {
...
switch ${dopant} {
    boron {
        # Substitutional boron: active minus BI pairs (no BV)
        term name = boronSub add eqn = "boronActive - boronInt" Silicon

        # Reaction term: Krate * (Sub * Defect - Pair / Binding)
        term name = ReactBI add eqn = "$KrateBI * (boronSub * Inter - boronInt / $Bind_BI)" Silicon

        # Pair diffusivity: (D0 + Dp*Poni) / (Binding * EqDefect * Poni)
...
switch ${dopant} {
    boron {
        # Inter: no IV recombination (no Vac PDE), BI pair coupling + clustering
        pdbSetString Silicon Inter Equation \
            "ddt(Inter) - $DiffI*EqInter*grad(ScaleInter) + ReactBI + R_clust"

        # Total boron: no direct flux, only BI pair reaction source/sink
...
# Vac surface recombination only for As (B dropped in noVac, P is I-only)
switch ${dopant} {
    arsenic {
