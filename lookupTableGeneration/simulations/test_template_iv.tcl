...
#
# Parameters:
#   boron      - boron, phosphorus, or arsenic
#   2e15        - implant dose (cm^-2)
#   20      - implant energy (keV)
#   1000        - anneal temperature (C)
#   30        - anneal time (minutes)
#
# Physics:
...
pdbSetString Silicon boron Equation "ddt(boron) - ($DiffDopantI) * (Inter/CIStar) * grad(boron) - ($DiffDopantV) * (Vac/CVStar) * grad(boron)"
...
set ramprate [expr {(1000 - 800.0) / 2700.0}]

puts "=== RAMP UP: 45 min, 800C -> 1000C ==="
diffuse time=45 temp=800 ramprate=$ramprate

puts "=== DWELL: 30 min at 1000C ==="
diffuse time=30 temp=1000

puts "=== RAMP DOWN: 45 min, 1000C -> 800C ==="
diffuse time=45 temp=1000 ramprate=-$ramprate
